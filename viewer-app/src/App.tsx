import type { ViewerContextPayload, ViewerDocumentPayload } from "@stakme/mdf/viewer-types";
...
export default function App(): JSX.Element {
        ...
        const fetchContext = useCallback(
                async (preserveSelection: boolean): Promise<ViewerContextPayload | null> => {
                        try {
        ...
        useEffect(() => {
                ...
                if (route.routePath === null) {
                        const defaultId = context.defaultDocumentId ?? context.documents[0]?.id ?? null;
                        if (!defaultId) {
                                setSelectedId((previous) => (previous === null ? previous : null));
                                return;
                        }

                        setSelectedId((previous) => (previous === defaultId ? previous : defaultId));
                ...
                if (targetId) {
                        setSelectedId((previous) => (previous === targetId ? previous : targetId));
                        return;
                }
        ...
        useEffect(() => {
                ...
                const handleReload = () => {
                        void fetchContext(true).then((payload) => {
                                ...
                                const documentExists = payload?.documents.some((entry) => entry.id === currentId);
        ...
        const shouldShowDocumentFields = useMemo(() => {
                ...
                if (activeDocument.visibleFields !== null && activeDocument.visibleFields.length === 0) {
                        return false;
                }
        ...
        const markdownButtonClassName = [toolbarButtonBase, toolbarButtonDefaultState].join(" ");
                        ...
                        <div className="flex flex-1 flex-col md:flex-row">
                                ...
                                <main className="flex-1 overflow-y-auto px-4 py-6 md:px-8 md:py-8">
                                        ...
                                        {route.type === "fm-field" && context && (
                                                <FrontMatterFieldView field={route.field} fields={context.frontMatter} />
                                        )}
